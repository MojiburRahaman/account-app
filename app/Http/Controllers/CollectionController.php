...
class CollectionController extends Controller
{
    ...
    function BackendhomeSUbmit(Request $request)
    {
        // return $request;
        $request->validate([
        ...
        if ($request->Bank == 1) {
            ...
            $payemnt->save();
            return redirect()->route('collectioncreate')->with('success', 'Payment Successfull');
        }
        if ($request->Bank == 3) {
            $request->validate([
        ...
        return redirect()->route('collectioncreate')->with('delete', 'Failed');
    }
}
